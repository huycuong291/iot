...
import { cost_last_month, cost_this_month, cost_this_day, cost_last_day, cost_last_year, cost_this_year, todayDeviceTreeSplit, mDeviceTreeSplit, yDeviceTreeSplit } from '../cost/cost_data1';
import CostTableComponent from './cost_table_component';
import { todayArr, todayDeviceOneSplit, todayDeviceTwoSplit, monthArr, mDeviceOneSplit, mDeviceTwoSplit, yearArr, yDeviceOneSplit, yDeviceTwoSplit } from '../cost/cost_data1';
    ...
    onClickbutton1() {
        ...
        if (window.selectedperiod === "today") {
            ...
            for (var i = 0; i < parseInt(moment().format('H')); i++) {
                // eslint-disable-next-line
                sftVal = sftVal + parseFloat(todayDeviceOneSplit[i]) + parseFloat(todayDeviceTwoSplit[i]) + parseFloat(todayDeviceTreeSplit[i]);
            }
            ...
            for (var i = 0; i < parseInt(moment().format('D')); i++) {
                sfmVal = sfmVal + parseFloat(mDeviceOneSplit[i]) + parseFloat(mDeviceTwoSplit[i]) + parseFloat(mDeviceTreeSplit[i]);
            }
            ...
            for (var i = 0; i < parseInt(moment().format('M')); i++) {
                styVal = styVal + parseFloat(yDeviceOneSplit[i]) + parseFloat(yDeviceTwoSplit[i]) + parseFloat(yDeviceTreeSplit[i]);
            }
